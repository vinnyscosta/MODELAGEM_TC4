...
class LSTMModel:
    ...
    def save(self) -> None:
        ...
        model_filename = f"models/{path}/lstm_model_{time}.keras"
        scaler_filename = f"models/{path}/scaler_{time}.pkl"
    ...
    def predict(
        ...
        # Caminho do modelo
        path = Path(os.path.join('models', ticker, model_folder))

        # Listar arquivos na pasta
        print(f"Arquivos disponíveis na pasta {path}: {list(path.glob('*'))}")

        # Identificar o arquivo do modelo e scaler
        try:
            model_file = next(path.glob("*.keras"))
        except StopIteration:
            raise FileNotFoundError(f"Nenhum arquivo .keras encontrado em {path}")

        try:
            scaler_file = next(path.glob("*.pkl"))
        except StopIteration:
            raise FileNotFoundError(f"Nenhum arquivo .pkl encontrado em {path}")
        ...
        recent_data = scaler.transform(recent_data.reshape(-1, 1))  # Transformação antes da predição  # noqa
        recent_data = recent_data.reshape(1, 3, 1)  # Ajusta a forma após a transformação  # noqa
